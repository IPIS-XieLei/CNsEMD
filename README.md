...
The public release will include expert reference labels and fixed training/test
split files. Neither the original T1-weighted (T1w) images nor the derived
direction-encoded color (DEC) images will be redistributed through CNsEMD.
Users should obtain the corresponding T1w and diffusion-weighted imaging (DWI)
data from the official Human Connectome Project (HCP) and Diff5T repositories,
then generate DEC images from the DWI data by following the MRtrix3 instructions
provided below.
...
The CNsEMD dataset will be deposited in ScienceDB:
...
After release, the ScienceDB deposit will contain expert reference labels and
the fixed training/test split files. The pretrained five-fold checkpoints for
PHM-Net, CNTSeg, and CNTSeg-v2 will instead be uploaded to the
`CNsEMD_weights/` directory in this GitHub repository after acceptance of the
associated paper.

Neither T1w nor DEC images are included in the ScienceDB deposit. Users should
obtain the corresponding T1w and DWI data from the official HCP and Diff5T
repositories, place the T1w images in the expected `T1/` directory, and derive
the DEC images from DWI using the MRtrix3 workflow described below. For
multimodal training or inference, the resulting T1w and DEC images must be
correctly paired and spatially co-registered.
...
The data directory expected by the provided scripts is organized as follows.
Neither the `T1/` nor the `DEC/` image content is included in the
ScienceDB deposit. Users must obtain T1w and DWI data from the official HCP and
Diff5T repositories and generate DEC images from DWI using the instructions
below.

```text
download_root/
└── CNsEMD/
    ├── T1/                         # User-provided from the source databases
    │   └── <case>-T1.nii.gz
    ├── DEC/                        # User-generated from the original DWI
    │   └── <case>-DEC.nii.gz
...
`3_100206-label.nii.gz` represent the same 3T case. Only the expert label is
included in the ScienceDB release. The corresponding T1w and DWI data must be
obtained separately from the applicable source repository; place the T1w image
in the `T1/` directory and generate the DEC image from DWI before placing it in
the `DEC/` directory.
...
└── CNsEMD_weights/               # Uploaded after acceptance
    ├── PHMNet/
...
After acceptance of the associated paper, download the pretrained weights from
the `CNsEMD_weights/` directory in this GitHub repository. Keep the directory
beside `predict_single_case.py`, or specify its location using `--result-root`.
...
- The `DEC/` directory must be populated with DEC images generated from the
  corresponding original DWI data using the MRtrix3 workflow below.
...
DEC images are not included in the CNsEMD public release. Users must generate
compatible DEC images from the original DWI data using
...
The CNsEMD dataset will be deposited in ScienceDB under DOI
...
The ScienceDB release will include expert reference labels and fixed
training/test split files. Pretrained model weights will be uploaded to the
`CNsEMD_weights/` directory in this GitHub repository after acceptance of the
associated paper. Neither T1w nor DEC images will be redistributed through
CNsEMD. Users should obtain the corresponding T1w and DWI data from the
official HCP and Diff5T repositories, place the T1w images in the `T1/`
directory, and generate DEC images from DWI using the MRtrix3 workflow
described above.
...
include expert reference labels and fixed data split files. Neither the
original T1w images nor DWI-derived DEC images are included.
