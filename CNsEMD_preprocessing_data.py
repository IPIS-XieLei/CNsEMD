...
def get_parser():
    ...
    parser.add_argument('--test_split_json', type=str,
                        default="/path/to/CNsEMD/splits_final_test.json")
    parser.add_argument('--output_root', type=str,
                        default="/path/to/CNsEMD_preprocessed_data")
    parser.add_argument('--modality_type', type=str, default="two", choices=["five", "two"],
...
def process_one_case(prefix_in_json, t1_dir, t2_dir, fa_dir, dec_dir, pk_dir, gt_dir, save_dir, modality_type="five", augment=False):
    ...
    for z in range(t1.shape[2]):
        ...
        if augment:
            ...
            save_nii(np.flip(t1_z, 0), aff, os.path.join(save_dir, "x_t1_data", f"{slice_aug}.nii.gz"))
            save_nii(np.flip(dec_z, 0), aff, os.path.join(save_dir, "x_dec_data", f"{slice_aug}.nii.gz"))
            save_nii(np.flip(lab_z, 0), aff, os.path.join(save_dir, "y_label_data", f"{slice_aug}.nii.gz"))
            if modality_type == "five":
                save_nii(np.flip(t2_z, 0), aff, os.path.join(save_dir, "x_t2_data", f"{slice_aug}.nii.gz"))
                save_nii(np.flip(fa_z, 0), aff, os.path.join(save_dir, "x_fa_data", f"{slice_aug}.nii.gz"))
                save_nii(np.flip(pk_z, 0), aff, os.path.join(save_dir, "x_peaks_data", f"{slice_aug}.nii.gz"))
            saved += 1
...
def process_fold(fold_idx, train_list, val_list, args):
    ...
    for name in train_list:
        ...
        process_one_case(name, t1_dir, t2_dir, fa_dir, dec_dir, pk_dir, gt_dir, train_save,
                         modality_type=args.modality_type, augment=True)
