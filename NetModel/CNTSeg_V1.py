...
class UNet2D(nn.Module):
    ...
    def forward(self, inputs):                  #  in->out       1*512*512      1*64*64
        ...
        return final
class DatafusionNet_2(nn.Module):
    def __init__(self, in_channels_1=10, n_classes=5, is_deconv=True, is_batchnorm=True):
        super(DatafusionNet_2, self).__init__()
        ...
        self.Conv2d3x3 = nn.Sequential(nn.Conv2d(self.in_channels_1, filters_base, kernel_size=3, stride=1, padding=1),
                                       ...
                                       nn.ReLU(inplace=True),
                                       nn.Conv2d(filters_base, filters_base, kernel_size=3, stride=1, padding=1),
                                       # nn.BatchNorm2d(filters_base),
                                       nn.ReLU(inplace=True)
                                       )
    ...
    def forward(self, inputs1, inputs2):
        ...
        conv3x3 = self.Conv2d3x3(intput)
        maxpool_1 = self.maxpool(conv3x3)
        conv3x3_2 = self.Conv2d3x3_2(maxpool_1)
        up2d2x2 = self.upConv2d2x2(conv3x3_2)
        conv3x3_3 = self.Conv2d3x3_2(up2d2x2)
        datafusion = self.datafusionfinal (conv3x3_3)

        return datafusion
class DatafusionNet_3(nn.Module):
    def __init__(self, in_channels_1=15, n_classes=5, is_deconv=True, is_batchnorm=True):
        super(DatafusionNet_3, self).__init__()
    ...
    def forward(self, inputs1, inputs2, inputs3):
        ...
        return datafusion
class DatafusionNet_5(nn.Module):
    def __init__(self, in_channels_1=25, n_classes=5, is_deconv=True, is_batchnorm=True):
        super(DatafusionNet_5, self).__init__()
    ...
    def forward(self, inputs1, inputs2, inputs3, inputs4, inputs5):
        intput = torch.cat([inputs1, inputs2, inputs3, inputs4, inputs5], 1)
        conv3x3 = self.Conv2d3x3(intput)
        maxpool_1 = self.maxpool(conv3x3)
        conv3x3_2 = self.Conv2d3x3_2(maxpool_1)
        up2d2x2 = self.upConv2d2x2(conv3x3_2)
