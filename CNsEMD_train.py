...
def get_parser():
    ...
    parser.add_argument("--model", type=str, default="PHMNet",
                        choices=["PHMNet", "CNTSeg_V1_T1DEC", "CNTSegV2_Dedicated"])
    ...
    parser.add_argument("--folds", type=str, default="fold_0,fold_1,fold_2,fold_3,fold_4")
    return parser.parse_args()

def calculate_metrics(pre_path, gt_case_path, file_prefix):
    gt_all_path = os.path.join(gt_case_path, f"{file_prefix}-label.nii.gz")
    gt_img = nib.load(gt_all_path)
    gt_all = gt_img.get_fdata().astype(np.int16)
    original_affine = gt_img.affine
    spacing = nib.affines.voxel_sizes(original_affine)
    ...
    def compute_hd(gt, pred):
        try:
            if gt.sum() > 0 and pred.sum() > 0:
                gt_itk = sitk.GetImageFromArray(gt.transpose(2, 0, 1).astype(np.int16))
                gt_itk.SetSpacing(spacing)
                pred_itk = sitk.GetImageFromArray(pred.transpose(2, 0, 1).astype(np.int16))
                pred_itk.SetSpacing(spacing)
                hd = sitk.HausdorffDistanceImageFilter()
                hd.Execute(gt_itk, pred_itk)
                return hd.GetAverageHausdorffDistance()
            else:
                return np.nan
        except Exception as e:
            print(f"HD compute error: {e}")
            return np.nan
    ...
    def compute_metrics(gt, pred):
        ...
        return dice, jac, prec


    metrics = []
    for c in [1, 2, 3, 4]:  # ON, OCN, TGN, FVN
        gt_c = (gt_all == c).astype(np.float32)
        pre_c = (pre == c).astype(np.float32)
        dice, jac, prec = compute_metrics(gt_c, pre_c)
        ahd = compute_hd(gt_c, pre_c)
        metrics.extend([dice, jac, prec, ahd])
    return metrics
...
def predict_and_evaluate(model, val_patch_path, save_path, case_list, args, device):
    ...
    with open(log_file, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("Case\tDice_ON\tJac_ON\tPrec_ON\tASD_ON\t"
                "Dice_OCN\tJac_OCN\tPrec_OCN\tASD_OCN\t"
                "Dice_TGN\tJac_TGN\tPrec_TGN\tASD_TGN\t"
                "Dice_FVN\tJac_FVN\tPrec_FVN\tASD_FVN\n")
        f.write("=" * 80 + "\n")
    ...
    if all_metrics:
        arr = np.array(all_metrics, dtype=np.float32)
        ...
        for m, s in zip(mean_vals, std_vals):
            mean_std_str.append(f"{m:.4f}±{s:.4f}")
        print("\n" + "=" * 80)
        print("FINAL SUMMARY (MEAN ± STD)")
        print("=" * 80)
        print(f"Mean Dice: ON={mean_vals[0]:.4f}, OCN={mean_vals[4]:.4f}, TGN={mean_vals[8]:.4f}, FVN={mean_vals[12]:.4f}")
        print(
            f"Mean Dice: CNs={((mean_vals[0]+mean_vals[4]+mean_vals[8]+mean_vals[12])/4):.4f}")
        print("=" * 80)
        with open(log_file, 'a') as f:
            f.write("\n" + "=" * 80 + "\n")
            f.write("MEAN±STD\t" + "\t".join(mean_std_str) + "\n")
            f.write("=" * 80 + "\n")
